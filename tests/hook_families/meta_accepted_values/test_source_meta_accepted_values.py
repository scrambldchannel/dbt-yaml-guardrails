"""Tests for source-meta-accepted-values CLI (specs/hook-families/meta-accepted-values.md)."""
...
_SOURCES = _YAML / "meta_accepted_values" / "sources"
...
def _invoke(*args: str) -> subprocess.CompletedProcess[str]:
    return subprocess.run(
        [
            sys.executable,
            "-m",
            "dbt_yaml_guardrails.hook_families.meta_accepted_values.source_meta_accepted_values",
            *args,
        ],
        cwd=_REPO_ROOT,
        capture_output=True,
...
def test_cli_skips_no_sources_section() -> None:
    r = _invoke("--key", "domain", "--values", "sales", _shared("minimal_version2.yml"))
    assert r.returncode == 0
