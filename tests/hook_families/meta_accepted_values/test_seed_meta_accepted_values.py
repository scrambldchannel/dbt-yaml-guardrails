"""Tests for seed-meta-accepted-values CLI (specs/hook-families/meta-accepted-values.md)."""
...
_SEEDS = _YAML / "meta_accepted_values" / "seeds"
_SHARED = _YAML / "shared"


def _f(name: str) -> str:
    return str(_SEEDS / name)
...
def _invoke(*args: str) -> subprocess.CompletedProcess[str]:
    return subprocess.run(
        [
            sys.executable,
            "-m",
            "dbt_yaml_guardrails.hook_families.meta_accepted_values.seed_meta_accepted_values",
            *args,
        ],
        cwd=_REPO_ROOT,
        capture_output=True,
