"""Tests for seed-allowed-column-keys CLI (specs/hook-families/allowed-column-keys.md)."""
...
def _invoke(*args: str) -> subprocess.CompletedProcess[str]:
    return subprocess.run(
        [
            sys.executable,
            "-m",
            "dbt_yaml_guardrails.hook_families.allowed_column_keys.seed_allowed_column_keys",
            *args,
        ],
        cwd=_REPO_ROOT,
        capture_output=True,
...
def test_cli_disallowed_column_key() -> None:
    ...
    assert "seed 'bad_seed'" in r.stderr
