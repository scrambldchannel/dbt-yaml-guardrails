"""Tests for model-allowed-meta-keys CLI (specs/hook-families/allowed-meta-keys.md)."""
...
_MODELS = _YAML / "allowed_meta_keys" / "models"
...
def _invoke(*args: str) -> subprocess.CompletedProcess[str]:
    return subprocess.run(
        [
            sys.executable,
            "-m",
            "dbt_yaml_guardrails.hook_families.allowed_meta_keys.model_allowed_meta_keys",
            *args,
        ],
        cwd=_REPO_ROOT,
        capture_output=True,
...
def test_cli_passes_clean_with_required() -> None:
    r = _invoke("--required", "owner", _m("meta_clean.yml"))
    assert r.returncode == 0
    assert r.stderr == ""


def test_cli_forbidden_meta_key() -> None:
    r = _invoke("--forbidden", "bad_key", _m("meta_forbidden.yml"))
...
def test_cli_no_policy_noop() -> None:
    r = _invoke(_m("meta_forbidden.yml"))
    assert r.returncode == 0
    assert r.stderr == ""
