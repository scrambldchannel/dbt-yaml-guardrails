"""Tests for source-allowed-meta-keys CLI (specs/hook-families/allowed-meta-keys.md)."""
...
_SOURCES = _YAML / "allowed_meta_keys" / "sources"
...
def _invoke(*args: str) -> subprocess.CompletedProcess[str]:
    return subprocess.run(
        [
            sys.executable,
            "-m",
            "dbt_yaml_guardrails.hook_families.allowed_meta_keys.source_allowed_meta_keys",
            *args,
        ],
        cwd=_REPO_ROOT,
        capture_output=True,
...
def test_cli_skips_no_sources_section() -> None:
    r = _invoke("--required", "owner", _shared("minimal_version2.yml"))
    assert r.returncode == 0
