"""catalog-allowed-keys CLI (``specs/hook-families/allowed-keys.md``; allowlist in ``resource_keys``)."""
...
from .resource_keys import (
    ...
    CATALOG_LEGACY_KEY_MESSAGES,
)


def _extract_catalog_by_name(
    success: ParseSuccess,
) -> ParseError | Mapping[str, Mapping[str, Any]] | None:
    r = extract_catalog_entries(success)
    if isinstance(r, CatalogEntriesSkip):
        return None
...
def _run(
    ...
    if "name" in required:
        typer.echo(message_name_in_required(resource_plural="catalogs"), err=True)
        return 2
    ...
    rows = collect_violation_rows_for_property_paths(
        ...
        forbidden,
        CATALOG_ALLOWED_KEYS,
        legacy_key_messages=CATALOG_LEGACY_KEY_MESSAGES,
        extract_by_name=_extract_catalog_by_name,
        iter_entries=iter_catalog_entries,
    )
...
def main(
    ...
    forbidden: str = typer.Option(
        ...
        "--forbidden",
        help=(
            "Comma-separated keys that must not appear on a catalog entry "
            "(stricter than the fixed allowlist in specs/resource-keys.md § Catalogs)."
        ),
    ),
) -> None:
    """Validate top-level keys on each catalog entry."""
    code = _run(files, required, forbidden)
    raise typer.Exit(code)


def cli_main() -> None:
    """Entry point for the ``catalog-allowed-keys`` console script."""
    try:
        typer.run(main)
