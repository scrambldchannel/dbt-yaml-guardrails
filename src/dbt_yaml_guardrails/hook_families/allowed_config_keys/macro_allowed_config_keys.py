"""macro-allowed-config-keys CLI (``specs/hook-families/allowed-config-keys.md``)."""
...
from dbt_yaml_guardrails.hook_families.allowed_config_keys.resource_config_keys import (
    MACRO_CONFIG_ALLOWED_KEYS,
    MACRO_CONFIG_LEGACY_KEY_MESSAGES,
)
...
def _run(
    ...
    return run_allowed_config_keys_cli(
        ...
        required_csv,
        forbidden_csv,
        allowed=MACRO_CONFIG_ALLOWED_KEYS,
        legacy_key_messages=MACRO_CONFIG_LEGACY_KEY_MESSAGES,
        resource_kind="macro",
...
def main(
    ...
    forbidden: str = typer.Option(
        ...
        help=(
            ...
            "(stricter than the fixed allowlist in specs/resource-config-keys.md § Macros)."
        ),
    ),
) -> None:
    """Validate top-level keys under config on each macro entry."""
    code = _run(files, required, forbidden)
    raise typer.Exit(code)


def cli_main() -> None:
    """Entry point for the ``macro-allowed-config-keys`` console script."""
    try:
        typer.run(main)
