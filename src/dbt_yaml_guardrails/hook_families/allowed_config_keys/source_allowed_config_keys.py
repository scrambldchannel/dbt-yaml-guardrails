"""source-allowed-config-keys CLI (``specs/hook-families/allowed-config-keys.md``)."""
...
def _run(
    ...
    return run_allowed_config_keys_cli(
        ...
        required_csv,
        forbidden_csv,
        allowed=SOURCE_CONFIG_ALLOWED_KEYS,
        legacy_key_messages=SOURCE_CONFIG_LEGACY_KEY_MESSAGES,
...
def main(
    ...
    forbidden: str = typer.Option(
        ...
        help=(
            ...
            "(stricter than the fixed allowlist in specs/resource-config-keys.md § Sources)."
        ),
    ),
) -> None:
    """Validate top-level keys under config on each source entry."""
    code = _run(files, required, forbidden)
    raise typer.Exit(code)


def cli_main() -> None:
    """Entry point for the ``source-allowed-config-keys`` console script."""
    try:
        typer.run(main)
