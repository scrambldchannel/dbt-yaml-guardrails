"""seed-allowed-meta-keys CLI (``specs/hook-families/allowed-meta-keys.md``)."""
...
def main(
    ...
    """Validate keys on config.meta for each seed entry."""
    code = _run(files, required, forbidden, allowed)
    raise typer.Exit(code)


def cli_main() -> None:
    """Entry point for the ``seed-allowed-meta-keys`` console script."""
    try:
        typer.run(main)
