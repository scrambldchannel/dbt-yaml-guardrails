"""macro-tags-accepted-values CLI (``specs/hook-families/tags-accepted-values.md``)."""
...
def _run(files: list[Path], values: str) -> int:
    return run_tags_accepted_values_cli(
        ...
        values,
        resource_kind="macro",
        extract_by_name=_extract_macros_by_name,
...
def main(
    ...
    """Validate config.tags against --values per macro entry."""
    code = _run(files, values)
    raise typer.Exit(code)


def cli_main() -> None:
    """Entry point for the ``macro-tags-accepted-values`` console script."""
    try:
        typer.run(main)
