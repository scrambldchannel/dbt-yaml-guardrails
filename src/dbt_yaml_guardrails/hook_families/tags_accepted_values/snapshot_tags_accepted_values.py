"""snapshot-tags-accepted-values CLI (``specs/hook-families/tags-accepted-values.md``)."""
...
def main(
    ...
    """Validate config.tags against --values per snapshot entry."""
    code = _run(files, values)
    raise typer.Exit(code)


def cli_main() -> None:
    """Entry point for the ``snapshot-tags-accepted-values`` console script."""
    try:
        typer.run(main)
