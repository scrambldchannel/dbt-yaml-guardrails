...
+ **CI:** **do not** require manual pre-commit on **`tests/hook_sandbox/`** for merge gates unless the team explicitly adds a dedicated workflow step; keep **`uv run pytest`** the automated gate in **[`ci.yml`](../.github/workflows/ci.yml)**.
