...
**Arguments:** see **§ Pattern: `*-allowed-keys`**. For **`--required`**: **`name`** is always present for each catalog in dbt; do not list it in **`--required`**. **Allowed keys:** **`resource-keys.md`** § **Catalogs**, implemented as **`CATALOG_ALLOWED_KEYS`**. Nested keys under each **`write_integrations`** item are not validated by this hook. **`--check-config`:** **no effect** in v1 (there is no **`config:`** on catalog entries under the current **`CATALOG_ALLOWED_KEYS`** spec; see **§ Nested keys (`config`) and `--check-config`**). **`--check-columns`:** **no effect** (catalog entries have no `columns:` list).
