...
+ **`--check-columns`** — **one** boolean option; default **`true`**. Applies **only** to **`model-allowed-keys`**, **`seed-allowed-keys`**, and **`snapshot-allowed-keys`** (the hooks whose entries have a `columns:` list). Accept explicit **`true`** or **`false`**. When **`true`**, these hooks also validate **direct keys on each entry in `columns:`** using the default allowlists from **`resource-keys.md`** § **Column keys** (implemented as **`*_COLUMN_ALLOWED_KEYS`** in **`resource_keys.py`**). When **`false`**, no column-level key checks are performed. **`macro-allowed-keys`**, **`exposure-allowed-keys`**, and **`source-allowed-keys`** do **not** accept this flag (macro and exposure entries have no `columns:` list; source columns are nested deeper and out of scope until `source-table-allowed-keys` is added).
+ **`--fix-legacy-yaml`** — **one** boolean option; default **`false`**. **Only** on the six property-YAML hooks **§§1–6** — **`model-allowed-keys`**, **`macro-allowed-keys`**, **`seed-allowed-keys`**, **`snapshot-allowed-keys`**, **`exposure-allowed-keys`**, **`source-allowed-keys`**. **§7 `catalog-allowed-keys`** and **§8 `dbt-project-allowed-keys`** **MUST NOT** expose this option (v1 rewrites are not defined for catalog or project file shapes; unrelated **`catalogs.yml`** / **`dbt_project.yml`** use pre-commit with different hooks, so there is no need for a no-op copy of the flag). **Opt-in** mechanical rewrite of legacy property-YAML keys **before** validation, as defined in **[`fix-legacy-yaml.md`](fix-legacy-yaml.md)** (v1: **`tests` → `data_tests`** at the declaration sites in that spec). When **`true`**, the hook **loads** the file with a **round-trip** editor (**ruamel.yaml**), **applies** the v1 rewrites **in place** (writing the file back when at least one in-scope rename occurs), then **re-reads** and runs the normal **`*-allowed-keys`** checks (required / allowlist / nested `config` / `columns` as configured). When **`false`**, behavior is **validation only** (unchanged; legacy keys are reported as violations, not renamed). **Conflict policy, key order, and out-of-scope paths** follow **`fix-legacy-yaml.md`**; if a conflict or unhandled error is reported for a node, that run **MUST** exit **non-zero** and **MUST NOT** treat the rewrite as successful for that node. **Document-wide rewrite:** when **`true`**, the rewrite pass applies to the **entire** property YAML document for each file this hook processes, not only the resource section this hook validates (e.g. **`model-allowed-keys`** may still rename **`tests`** under **`seeds:`** in the same file). **Future** transforms in **`fix-legacy-yaml.md`** (e.g. top-level **`tags` / `meta`**) apply when **`--fix-legacy-yaml` true** once implemented for a release. **User-facing documentation** (repository root **`HOOKS.md`**) **SHOULD** list which hook **`id`s** support this flag and **SHOULD** state explicitly that **catalog** and **dbt-project** **`id`s** do not.
...
**Arguments:** see **§ Pattern: `*-allowed-keys`** and **§ Nested keys (`config`) and `--check-config`**. For **`--required`**: **`name`** is always present for real sources in dbt; do not list it in **`--required`**. **Allowed keys:** **`resource-keys.md`** § **Sources**, implemented as **`SOURCE_ALLOWED_KEYS`**. **Nested `config` keys (when `--check-config` is on):** **`resource-config-keys.md`** § **Sources — default keys under `config`** / **`SOURCE_CONFIG_ALLOWED_KEYS`**, matching **`source-allowed-config-keys`**. **`--check-columns`:** **not accepted** in v1; source entry columns are nested under `tables: → [table] → columns:` — a deeper path addressed when `source-table-allowed-keys` is specced (see §9).
