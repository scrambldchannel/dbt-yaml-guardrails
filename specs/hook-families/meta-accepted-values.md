...
The snippets below are **`hooks:`** stanzas only. A full **`repos:`** / **`rev:`** example for this repository is in **[`HOOKS.md`](../../HOOKS.md)**; **`rev:`** must be **`v`** plus **`[project] version`** in **`pyproject.toml`** (currently **`v0.4.1`**).
