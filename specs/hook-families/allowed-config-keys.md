...
**Overlap with `*-allowed-keys`:** **[`hook-families/allowed-keys.md`](allowed-keys.md)** § **Nested keys (`config`) and `--check-config`** specifies that, by default, **`*-allowed-keys`** also checks **direct keys under `config:`** using **these same** default allowlists and legacy maps. The **`*-allowed-config-keys`** family **remains shipped**; it **may** be **deprecated** later in favor of relying on **`*-allowed-keys`** with **`--check-config` default `true`**, but that is **not** required for the v1 nested implementation. **Each hook runs independently** as configured: running **both** on the same paths **may** emit **two** stderr lines for the same unknown **`config:`** key (one per hook). Implementations **MUST NOT** deduplicate across hooks.
...
| Family | Validates |
| --- | --- |
| **`*-allowed-keys`** | Top-level keys on the **resource entry** (e.g. `models: [].`… keys like `name`, `config`, `columns`). When **`--check-config`** is **true** (default), also **direct keys under `config:`** with the same default allowlists as this family (see **[`allowed-keys.md`](allowed-keys.md)** § **Nested keys (`config`) and `--check-config`**). |
...
Same resource list as the **`*-allowed-keys`** wave (**`model`**, **`macro`**, **`seed`**, **`snapshot`**, **`exposure`**):
