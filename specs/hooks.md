...
| **`config.tags`** value shape (string or list of strings) | **`hook-families/tags-accepted-values.md`** | **`*-tags-accepted-values`**: **spec only** (not shipped yet); **`--values`** allowlist, optional **`--optional`**; no path flag—always **`config.tags`** |
