...
| String at a path under `config.meta` (v1) | **`hook-families/meta-accepted-values.md`** | **`*-meta-accepted-values`** (specified, not shipped): **`--key`** dot path, **`--values`** comma-separated allowlist, optional **`--optional`** for presence; other scalar types **future** — **preferred next implementation** for dot paths under **`meta`** (simpler than nested **`--allowed`** in **`*-allowed-meta-keys`**) |
