...
| Mechanical rewrites of deprecated property-YAML (legacy keys) | **`hook-families/dbt-yaml-legacy.md`** | **`dbt-yaml-legacy`**: v1 renames **`tests` → `data_tests`** at resource and column scope as defined there. Default: **check** (exit 1 if changes would be made); pass **`--write`** to apply. Future: top-level **`tags` → `config.tags`**, **`meta` → `config.meta`**. Complements `*-allowed-keys`; not a duplicate validator. |
