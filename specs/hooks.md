...
| Mechanical rewrites of deprecated property-YAML (legacy keys) | **`hook-families/fix-legacy-yaml.md`** | **`fix-legacy-yaml`**: v1 renames **`tests` → `data_tests`** at resource and column scope as defined there. Default: **check** (exit 1 if changes would be made); pass **`--write`** to apply. Future: top-level **`tags` → `config.tags`**, **`meta` → `config.meta`**. Complements `*-allowed-keys`; not a duplicate validator. |
