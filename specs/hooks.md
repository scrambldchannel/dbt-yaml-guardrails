...
| String at a path under `config.meta` (v1) | **`hook-families/meta-accepted-values.md`** | **`*-meta-accepted-values`**: **`model-meta-accepted-values`** shipped; **`--key`** dot path, **`--values`** comma-separated allowlist, optional **`--optional`** for presence; other resource CLIs and non-string scalars **future** |
