...
| --- | --- | --- |
| Top-level keys on each resource entry | **`hook-families/allowed-keys.md`** | Keys on each dict under `models:`, `macros:`, `seeds:`, `sources:`, `catalogs:`, … (**`source-allowed-keys`**, **`catalog-allowed-keys`**, and other `*-allowed-keys`; see that spec) |
