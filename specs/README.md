...
8. **[`hook-families/meta-keys-accepted-values.md`](hook-families/meta-keys-accepted-values.md)** — **`*-meta-key-accepted-values`** (specified; not shipped): string leaf at a dot path under **`meta`** (**`--key`**, **`--values`**, **`--optional`**); non-string scalars **later**. **Implementation priority:** ship this **before** nested dot paths in **`allowed-meta-keys.md`** (see both specs).
