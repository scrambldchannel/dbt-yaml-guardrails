...
4. **[`resource-keys.md`](resource-keys.md)** — Default allowed keys per resource type (Fusion-oriented); **`src/dbt_yaml_guardrails/resource_keys.py`** is the implementation source for **`model-allowed-keys`** (`MODEL_ALLOWED_KEYS`); extend both as new resource types are supported.
5. **[`hooks.md`](hooks.md)** — Catalog of hooks: CLI names, arguments, **exit codes**, defaults, **`.pre-commit-hooks.yaml`** packaging, and pre-commit integration per hook.
