...
9. **[`hook-families/fix-legacy-yaml.md`](hook-families/fix-legacy-yaml.md)** — **Mechanical legacy rewrites** (`fix-legacy-yaml` hook, check / `--write`). v1: **`tests` → `data_tests` only;** future: top-level **`meta` / `tags`** into **`config`**. Complements the validator families; does not replace them.
