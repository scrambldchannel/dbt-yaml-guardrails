...
10. **[`hook-families/meta-accepted-values.md`](hook-families/meta-accepted-values.md)** — **`*-meta-accepted-values`**: string or string list at a dot path under **`meta`** (**`--key`**, **`--values`**, **`--optional`**); **`model`**, **`seed`**, **`snapshot`**, **`exposure`**, **`macro`** shipped; non-string scalars **later** (see that spec).
11. **[`hook-families/tags-accepted-values.md`](hook-families/tags-accepted-values.md)** — **`*-tags-accepted-values`**: allowlist **`config.tags`** with **`--values`** only; **`model`**, **`seed`**, **`snapshot`**, **`exposure`**, **`macro`** shipped.
