...
4. **[`resource-keys.md`](resource-keys.md)** — Default **top-level** allowed keys per resource type for **`*-allowed-keys`** (Fusion-oriented); **`src/dbt_yaml_guardrails/hook_families/allowed_keys/resource_keys.py`** is the implementation source (e.g. **`MODEL_ALLOWED_KEYS`**, **`SOURCE_ALLOWED_KEYS`**, **`MACRO_ALLOWED_KEYS`**); extend both as new resource types are supported.
