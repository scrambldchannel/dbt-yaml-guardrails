...
7. **[`hook-families/allowed-keys.md`](hook-families/allowed-keys.md)** — **`*-allowed-keys`** hooks: pattern, **exit codes**, and each shipped or specified CLI (**`model`**, **`macro`**, **`seed`**, **`source`**, **`snapshot`**, **`exposure`**, **`catalog`**, …; **`dbt-project-allowed-keys`** in §8 is specified for **`dbt_project.yml`**; implementation TBD).
