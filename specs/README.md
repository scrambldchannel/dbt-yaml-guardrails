...
5. **[`resource-config-keys.md`](resource-config-keys.md)** — Default keys **under each entry’s `config:`** for **`*-allowed-config-keys`** (see **`hook-families/allowed-config-keys.md`**); implementation constants **TBD**.
...
8. **[`hook-families/allowed-config-keys.md`](hook-families/allowed-config-keys.md)** — **`*-allowed-config-keys`**: keys under each resource’s **`config`** mapping; same CLI pattern as **`*-allowed-keys`**; default allowlists in **`resource-config-keys.md`**; **specified**, not shipped.
