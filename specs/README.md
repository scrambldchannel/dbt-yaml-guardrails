...
8. **[`hook-families/meta-accepted-values.md`](hook-families/meta-accepted-values.md)** — **`*-meta-accepted-values`**: string leaf at a dot path under **`meta`** (**`--key`**, **`--values`**, **`--optional`**); **`model`**, **`seed`**, **`snapshot`**, **`exposure`** shipped; **`macro`** and non-string scalars **later**.
