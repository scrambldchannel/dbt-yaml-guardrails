...
- **Dead code (Vulture) before release:** As part of the same pre-ship check as tests and **`CHANGELOG`**, run **`make vulture`** from the repository root (see root **`Makefile`**). That target runs Vulture with **`--config pyproject.toml`**, so **`[tool.vulture]`** (paths, confidence, `ignore_names`, …) is always applied. **Do not** hand-type a different Vulture command line for releases—the **Makefile** and config are the source of truth. The run **SHOULD** exit **0** before the version-bump commit is merged to **`main`**.
